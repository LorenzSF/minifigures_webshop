...
def main():
    ...
    image_slice = image_tags[
        st.session_state.current_page * 9 : min(
            st.session_state.current_page * 9 + 9, len(image_tags)
        )
    ]
...
def _open_product(image_tag: str) -> None:
    """Persist selected item and open the Product page."""
    st.session_state.selected_product_tag = image_tag
    if hasattr(st, "switch_page"):
        st.switch_page("pages/0_🎭_Product.py")
        return
    st.info("Selected item saved. Open the Product page from the sidebar.")
