...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, tag: str, classes: list[str], resolution: int = 256):
    ...
    def get_metadata(self) -> dict[str, Any]:
        ...
        return {"tag": self.tag, "classes": self.classes, "resolution": self.resolution}
    ...
    def save(self):
        ...
        with open(mdl_f / "metadata.json", "w") as f:
            json.dump(self.get_metadata(), f, indent=2, sort_keys=True)
    ...
    def load(cls, tag: str):
        """Load the model."""
        # Load metadata first
        with open(get_models_folder() / tag / "metadata.json") as f:
            metadata = json.load(f)

        # Create the model
        model = cls(**metadata)

        # By default in eval mode
        model.eval()
