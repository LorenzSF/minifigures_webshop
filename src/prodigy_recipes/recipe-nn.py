...
from data.data import (
    OPTIONS,
    generate_caption,
    generate_multiple_caption_probs,
    get_image,
    get_latest_model,
    predict_dataset,
)
...
@prodigy.recipe("minifigures.nn", k=("Number of nearest neightbors", "option", "k", int))
def image_caption(k: int = 5):
    ...
    dataset = MinifiguresDataset(
        data_f=get_data_folder(), resolution=256, balance=False, augment=False, training=training
    )
    ...
    def load_stream(images_path):
        ...
        idx_lbld = np.array(
            [i for i, image in enumerate(stream) if image["text"].replace(".png", "") in known_imgs]
        )
        _, y_true, _ = predict_dataset(dataset, model)
        X_all = np.array(
            [
                np.ravel(get_image(image["text"], max_w=200, max_h=400).cpu().detach().numpy())
                for image in stream
            ]
        )
