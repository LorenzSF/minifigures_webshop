...
import numpy as np
import prodigy
from data.data import OPTIONS, generate_caption, generate_multiple_caption_probs, get_latest_model
...
@prodigy.recipe("minifigures.confused")
def image_caption():
    ...
    def load_stream(images_path):
        ...
        confusion = np.sum(np.abs(y_preds - 0.5), axis=1)
