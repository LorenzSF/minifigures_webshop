...
import prodigy
from data.data import OPTIONS, generate_caption, get_already_labeled, get_latest_model
...
@prodigy.recipe(
    "minifigures.category", n=("How many least present classes to include", "option", "n", int)
)
def image_caption(n: int | None = 1):
    ...
    def load_stream(images_path):
        ...
        labeled_frac_per_keyword = {
            ...
            / len([im for im in stream if key in im["text"]])
            for key in keywords
        }
